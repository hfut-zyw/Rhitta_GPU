...
class SRN(Module):
    def __init__(self, input_size, hidden_size, time_dimension):
        ...
        # 初始化模型参数
        self.rnncell = RNNCell(self.input_size, self.hidden_size)
    ...
    def __call__(self, inputs, init_state):
        """
        接收一个时间序列节点的列表，和初始状态节点
        """
        hidden_state, _ = self.rnncell(inputs[0], init_state)  # 先送进初始状态获取第一层隐藏状态
        N = self.time_dimension - 1
        for i in range(N):
            hidden_state, _ = self.rnncell(inputs[i + 1], hidden_state)
        return hidden_state  # 输出最后一个隐藏状态的节点
