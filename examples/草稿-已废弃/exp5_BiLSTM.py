...
model = nn.BiLSTM(input_size=1, hidden_size=3, time_dimension=4)
# W,b是用于将输出经过一个线形层，将维度变成标签维度
W = nn.to_tensor(size=(6, 1), require_gradient=True, initializer=initializer)
b = nn.to_tensor(size=(1, 1), require_gradient=True, initializer=initializer)

"""
挖坑：
h_0:一个to_tensor节点，用于接收正向初始状态
c_0:一个to_tensor节点，用于接收正向初始状态
h_1:一个to_tensor节点，用于接收反向初始状态
c_1:一个to_tensor节点，用于接收反向初始状态
...
h_1 = nn.to_tensor(size=(1, 3))
c_1 = nn.to_tensor(size=(1, 3))
...
h_out = model(inputs, h_0, c_0, h_1, c_1)
output = h_out * W + b
...
# 初始状态的值设一次即可，后面不会变化或消失
h_0.set_value(np.zeros((1, 3)))
c_0.set_value(np.zeros((1, 3)))
h_1.set_value(np.zeros((1, 3)))
c_1.set_value(np.zeros((1, 3)))
...
for epoch in range(epochs):
    ...
    for i in range(N_train):
        ...
        if count >= batch_size:
            ...
            count = 0
    axes_x.append(epoch)
    axes_y.append(loss.value.getA()[0][0])
